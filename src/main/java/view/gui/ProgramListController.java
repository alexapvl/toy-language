...
public class ProgramListController {
  ...
  public void initialize() {
    ...
    for (String key : keySet) {
      programDescriptions.add(String.format("%4s : %s", textMenu.getCommands().get(key).getKey(), textMenu.getCommands().get(key).getDescription()));
    }
    programListView.setItems(programDescriptions); // set the program descriptions
  }
  ...
  public void executeProgram() {
    ...
    try {
      ...
      // uncomment line bellow if you want to execute only one program / run
